...
def land_find(ranges,angles,prom):
    ...
    left_bases = prominence['left_bases']
    right_bases = prominence['right_bases']
    """mask_prom = prominence['prominences'] <= 1 #to account for inf drops
    peaks = peaks[mask_prom]"""
    print('peaks', peaks)
    print('--')
    left_base_values = ranges_new[left_bases]
    right_base_values = ranges_new[right_bases]
    higher_base_indices = np.maximum(left_base_values, right_base_values)
    condition_met_mask = higher_base_indices > 4
    print("Mask (True if higher base index > 4):", condition_met_mask)
    if condition_met_mask.any():
        print("\n--- Peaks where the higher base INDEX is > 4 ---")
        for i in range(len(peaks)):
            if condition_met_mask[i]:
                np.delete(peaks,i)

    print('----')
    print('peaks',peaks)
    print('otherpeaks', )
    #infin = np.where(np.isinf(ranges))[0]
    inf_intervals = find_inf_intervals(ranges)
    for j in range(len(inf_intervals)):
        print(inf_intervals[j])
    ...
    for m in range(len(peaks)):
        ...
        if peaks[m] + previous_choice >= len(ranges_new) - 1:
            previous_choice = len(ranges_new) - peaks[m] - 1
        elif peaks[m] - previous_choice < 0:
            previous_choice = peaks[m]


        take_previous = peaks[m] - previous_choice
        mp_prev = ranges_new[take_previous]
        ma_prev = angles_new[take_previous]
        take_next = peaks[m] + previous_choice
        mp_next = ranges_new[take_next]
        ma_next = angles_new[take_next]

        #den = abs(ma_prev - ma_current) + 1e-10 #avoid dividing by zero
        den = previous_choice/3 * np.pi / 180 #radians
    ...
    ax1.plot(angles_new, ranges_new, label=r'ranges(0)$')
    for x in peaks:
        plt.scatter(angles_new[x], ranges_new[x], color='r')
    for h in range(len(left_bases)):
        plt.scatter(angles_new[left_bases[h]], ranges_new[left_bases[h]], color='b')
        plt.scatter(angles_new[right_bases[h]], ranges_new[right_bases[h]], color='y')
    print(len(angles_new), len(ranges_new))
    plt.show()
...
#Odometry trajectory
Ts = time_stamps[1]-time_stamps[0]
